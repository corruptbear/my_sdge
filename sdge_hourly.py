...
def stacked_plot(dates=None, daily=None):
    daily_super_offpeak = []
    daily_offpeak = []
    daily_peak = []
    ...
    for date, consumption_data in daily.items():
        ...
        daily_super_offpeak.append(sum([consumption_data[hour] for hour in get_tou_hours(d)['SUPER_OFFPEAK']]))
        daily_offpeak.append(sum([consumption_data[hour] for hour in get_tou_hours(d)['OFFPEAK']]))
        daily_peak.append(sum([consumption_data[hour] for hour in get_tou_hours(d)['PEAK']]))
    ...
    plt.bar(dates, daily_super_offpeak, label='super offpeak', color = 'C0')
    plt.bar(dates, daily_offpeak, bottom = daily_super_offpeak, label='offpeak', color = 'C2')
    plt.bar(dates, daily_peak, bottom = daily_super_offpeak + daily_offpeak, label='peak', color = 'C1')
...
def aggregated_hourly_plot(dates=None, daily=None):
...
def daily_hourly_2d_plot(daily=None, dates=None):
...
def daily_hourly_3d_plot(daily=None,  dates=None):
    if (len(daily.index)>=50):
        return
...
def plot_sdge_hourly(filename):
    ...
    days = [pd.to_datetime(x[0],'%Y-%m-%d').date() for x in daily.items()]

    #plot the daily summary bar plot without stacking
    #plt.bar(days,daily_summary.values)

    stacked_plot(dates=days, daily=daily)

    #plot day by day
    daily_hourly_2d_plot(daily=daily, dates=days)
    daily_hourly_3d_plot(daily=daily, dates=days)

    #plot hourly data summed across days
    aggregated_hourly_plot(daily=daily, dates=days)
